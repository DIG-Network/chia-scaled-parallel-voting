...
**What this doc is:** how **Ballot Coin `finalize`** checks a Groth16 proof plus aggregate **BLS** in CLVM ([CHIP-0011](https://github.com/Chia-Network/chips/blob/main/CHIPs/chip-0011.md)), **why** that combination is sound, and **informative** context on prover cost and a possible **BLS12-377** future. **Threshold intuition:** figures in the reference repo’s [`assets/`](https://github.com/DIG-Network/chia-parallel-voting/tree/main/assets) tree (not literal CRS diagrams). Image URLs below use `raw.githubusercontent.com` so they render when this Markdown is viewed from the Chia chips repository without a local `assets/` copy.

**Code:** [DIG-Network/chia-parallel-voting](https://github.com/DIG-Network/chia-parallel-voting) on `main`. **Public-input order and Merkle rules:** [chip-witnesses-encoding.md](./chip-witnesses-encoding.md). **Spec overview:** [CHIP_DRAFT.md](./CHIP_DRAFT.md).
...
**Ballot `finalize`** ([`puzzles/ballot_coin/finalize.rue`](https://github.com/DIG-Network/chia-parallel-voting/blob/main/puzzles/ballot_coin/finalize.rue)) does the following in order:
...
<figure>
  <img
    src="https://raw.githubusercontent.com/DIG-Network/chia-parallel-voting/main/assets/figure_2.png"
