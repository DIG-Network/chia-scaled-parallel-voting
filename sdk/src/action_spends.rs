...
/// FN: build_ceremony_finalizer_full
/// WHAT: produce the FULLY-curried Ceremony Singleton finalizer
///       puzzle reveal. Mirrors [`build_election_finalizer_full`] /
///       [`build_ballot_finalizer_full`] but loads the Ceremony
///       Singleton finalizer hex and uses `ceremony_launcher_id` for
///       the HINT.
/// USAGE: pass to `build_action_layer_puzzle` when assembling the
///        Ceremony Singleton's inner action layer (currently only
///        the `contribute` action).
pub fn build_ceremony_finalizer_full(
    ctx: &mut SpendContext,
    ceremony_launcher_id: Bytes32,
) -> VotingResult<NodePtr> {
    use crate::puzzles::{PuzzleHashes, CEREMONY_SINGLETON_FINALIZER_HEX};
    let bytes = hex::decode(
        CEREMONY_SINGLETON_FINALIZER_HEX
            .trim()
            .trim_start_matches("0x"),
    )
    .map_err(|e| voting_other(format!("decoding CEREMONY_SINGLETON_FINALIZER_HEX: {e}")))?;
    let program = Program::from(bytes);
    let program_node = program
        .to_clvm(&mut **ctx)
        .map_err(|e| voting_other(format!("loading ceremony finalizer: {e}")))?;

    let action_layer_mod = PuzzleHashes::action_layer();
    let first_curry = CurriedProgram {
        program: program_node,
        args: clvm_curried_args!(action_layer_mod, ceremony_launcher_id),
    }
    .to_clvm(&mut **ctx)
    .map_err(|e| voting_other(format!("first-currying ceremony finalizer: {e}")))?;
    ...
    .to_clvm(&mut **ctx)
    .map_err(|e| voting_other(format!("second-currying ceremony finalizer: {e}")))?;
    Ok(full)
}
